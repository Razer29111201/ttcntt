...
const getadmin = async (req, res, next) => {
    ...
    const [sdtc] = await pool.execute('SELECT * FROM `sdtc` ORDER BY id DESC');
    console.log(sdtc);
    return res.render('admin.ejs', { acc: acc[0], slide: slide[0], news: news[0], ttvb: ttvb, sdtc: sdtc })
}
...
const gtdetail = async (req, res) => {
    var role = req.params.role
    const [id] = await pool.execute('select * from sdtc where role = ?', [role])
    console.log(id);
    return res.render('gtdetail.ejs', { role: id })

}
const addsdtc = async (req, res) => {


    return res.render('addsdtc.ejs')


}
const sdtcadd = async (req, res) => {
    var file = req.file.path.split('\\').splice(2).join('/')
    console.log(file, req.body, req.file);

    pool.query('insert into sdtc (img, chucvu, tdcm,tdllct,sdtvp,sdtdt,email,role) values ( ?, ?,?,?,?,?,?,?)',
        [file, req.body.cv, req.body.tdcm, req.body.llct, req.body.dtvp, req.body.dtdd, req.body.Email, req.body.role]);
    res.redirect('/admin')
}
const geteditsdtc = async (req, res) => {
    var id = req.params.id
    const [sdtc] = await pool.execute('select * from sdtc where id = ?', [id])
    res.render('editsdtc.ejs', { sdtc: sdtc[0] })
}
...
const adduser1 = async (req, res) => {
    ...
    bcrypt.hash(pass, saltRounds, function (err, hash) {
        ...
        pool.query('insert into account (surname, name, user,Email,pass,sex,role) values ( ?, ?,?,?,?,?,?   )',
            [surname, name, user1, email, passbcry, sex, role]);
        return res.redirect('/admin')
    });
}
...
const edituser = async (req, res) => {
    var id = req.params.id
    const [acc] = await pool.execute('select * from account  where id = ?', [id])
    console.log(acc);
    res.render('edituser.ejs', { acc: acc })

}

const edituser1 = async (req, res) => {
    console.log(req.body);
    var user1 = req.body.user
    var surname = req.body.surname
    var name = req.body.name
    ...
    bcrypt.hash(pass, saltRounds, function (err, hash) {
        ...
        pool.query('update  account set surname =?, name =?, user =?,Email=?,pass=?,sex=?,role=? where id = ?',
            [surname, name, user1, email, passbcry, sex, role, id]);
        return res.redirect('/admin')
    });
}
